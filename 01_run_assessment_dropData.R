...
# Apply before conversion and q-splitting.
# ============================================================
...
# ============================================================
# Convert index types
# Auto-detect via catch.n (same pattern as 01_run_assessment.R).
# Chile_AcousCS, DEPM, Peru_Acoustic dropped above so only
# Chile_AcousN will be detected as number-at-age here.
# ============================================================

num_idx  <- unlist(lapply(idx_h1, function(x) sum(catch.n(x), na.rm = TRUE) > 0))
biom_idx <- !num_idx
...
# Chile_AcousCS, DEPM, and Peru_Acoustic are dropped from the input set
# so they are excluded from LOO groups.
loo_groups <- list(
